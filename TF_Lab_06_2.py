...
# 여기서 -1은 행은 1씩 퍼트리고, 열은 7만큼 배열을 끊어라.
# 뒤에는 7, 앞에는 알아서
# 그래서 여기서 [[1000000], [0001000]]으로 다시 돌아오게 된다.
# 여기가 우리가 원하는 차원, 모양을 맞춰줄 수 있다.

# reshape 보충설명
# reshape의 파라미터에 -1이 들어가면 특별한 의미를 갖는데, 다른 나머지 차원 크기를 맞추고 남은 크기를 해당 차원에 할당해 준다는 의미.
# 또는 다차원 배열의 원소수가 변환 후에도 똑같이 유지되도록 적절히 묶어줍니다.
# 약간 풀어해치는 느낌? 알아서 묶음을 만들어준다는 느낌.
